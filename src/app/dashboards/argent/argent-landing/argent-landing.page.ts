...
@Component({
  selector: 'app-argent-landing',
  templateUrl: './argent-landing.page.html',
  styleUrls: ['./argent-landing.page.scss'],
})
export class ArgentLandingPage implements OnInit {
  ...
  ngOnInit() {
    this.afStore.collection('orders',ref => ref.where('agentId', '==', this.userDetails.getUID())).valueChanges().subscribe(goalList=>{
  ...
  remove(item){
    ...
    this.navCtrl.navigateForward(['argent-order-details',{
